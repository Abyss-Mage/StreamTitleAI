  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (ytResult.status === 'fulfilled') setYtData(ytResult.value.data);
        if (twResult.status === 'fulfilled') setTwData(twResult.value.data);
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-400">Deep dive into your channel metrics.</p>
        </div>
        <div className="flex bg-surface border border-border rounded-lg p-1">
          <button onClick={() => setActiveTab('youtube')} className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-bold transition-all ${activeTab === 'youtube' ? 'bg-red-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}>
            <Youtube size={16} /> YouTube
          </button>
          <button onClick={() => setActiveTab('twitch')} className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-bold transition-all ${activeTab === 'twitch' ? 'bg-purple-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-6">
            ...
            ytData ? (
              <div className="space-y-6 animate-in fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  {renderStatCard("Net Subs", ytData.stats.netSubs, <Users className="text-green-400" size={24} />, "text-green-400")}
                  {renderStatCard("Watch Hours", ytData.stats.watchTimeHours, <Clock className="text-orange-400" size={24} />, "text-orange-400")}
                  {renderStatCard("Likes", ytData.stats.likes, <Heart className="text-pink-400" size={24} />, "text-pink-400")}
                </div>
              </div>
            ) : <div className="text-center py-20 text-slate-500">YouTube not connected.</div>
          )}
          ...
          {activeTab === 'twitch' && (
              ...
              <div className="space-y-6 animate-in fade-in">
                <div className="flex items-center gap-4 bg-gradient-to-r from-purple-900/20 to-surface border border-purple-500/20 p-6 rounded-2xl">
                  ...
                  <div>
                    ...
                    <span className="text-purple-400 text-sm font-medium">Twitch Streamer</span>
                  </div>
                </div>
            ...
            ) : <div className="text-center py-20 text-slate-500">Twitch not connected.</div>
          )}
        </div>
      )}
    </div>
