...
const { mainModel, optimizePrompt, outliersPrompt, keywordsPrompt } = require('../config/ai'); // <-- 1. IMPORT NEW PROMPT
...
aiRouter.post('/optimize', async (req, res) => {
    try {
        ...
        let creatorProfile = {};
        // Use subcollection logic
        const profileRef = db.collection('creatorProfiles').doc(uid).collection('profiles');
        const snapshot = await profileRef.limit(1).get();
        if (!snapshot.empty) {
            creatorProfile = snapshot.docs[0].data();
            console.log(`[StreamTitle.AI] Using profile: ${creatorProfile.name || 'default'}`);
        }
...
aiRouter.post('/discover/outliers', async (req, res) => {
    try {
        ...
        // 1. Fetch Creator Profile
        let creatorProfile = {};
        const profileRef = db.collection('creatorProfiles').doc(uid).collection('profiles'); // Use subcollection
        const snapshot = await profileRef.limit(1).get();
        if (!snapshot.empty) {
            creatorProfile = snapshot.docs[0].data();
        }
...
aiRouter.post('/discover/keywords', async (req, res) => {
    try {
        ...
        // As a default, let's try to find *any* profile if multi-profile is set up
        // A more robust solution might have the user select a profile on the Discover page
        const snapshot = await profileRef.limit(1).get();
        if (!snapshot.empty) {
            creatorProfile = snapshot.docs[0].data();
        }
